...
def dowellClusterSampling(clusterSamplingInput):
    ...
    n = clusterSamplingInput['M']                 # Initializing number of clusters
    ...
    # Identify clusters (e.g., 'City' in this example)
    df = clusterSamplingInput["sam"]
    clust = clusterSamplingInput["cluster"]
    cluster = clust.capitalize()
    clusters = df[cluster].unique()

    # Randomly select clusters (let's say we want to select 2 clusters)
    selected_clusters = pd.Series(clusters).sample(n, random_state=1).tolist()
    
    # Perform cluster sampling
    sampled_clusters = df[df[cluster].isin(selected_clusters)]

    # Display the cluster sample
    print("cluster is" ,sampled_clusters)
